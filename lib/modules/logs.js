	...
	modLogs = {
		...
		getStream: function() {
			...
			if (!this.stream) {
				if(isStream(this.visitLog)) {
					...
					var p = bonefs.pathResolve(this.visitLog);
					this.stream = bonefs.createWriteStream(p, {
						flags: 'a',
						encoding: 'utf8',
						focus: true
					});
		...
		getStreamByCt: function(contentType) {
			...
			if(isStream(this.responseLog)) {
				...
				var logFileName = bonefs.pathResolve(this.responseLog)+'.'+enc;

				this.rotateFile(logFileName);

				return bonefs.createWriteStream(logFileName, {
					flags: 'a',
					encoding: enc,
					focus: true
				});
