...
(function() {
	...
	modProxyRequest = function(originalRequest, originalResponse, macFiddler) {
		...
		reqCallback = function(proxyResponse) {
			...
			originalResponse.writeHead(proxyResponse.statusCode, proxyResponse.statusMessage, proxyResponse.headers);
	...
	modProxyRequest.connect = function(request, response, next) {
		var url = request.url;
		if (url.charAt(0) !== '/') {
			modProxyRequest(request, response, this.app);
		} else {
			next();
		}
	};
